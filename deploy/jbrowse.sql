...
COMMENT ON COLUMN configuration IS 'The json.conf file in json format';
...
create table jbrowse_organism (
    ...
    foregin key (jbrowse_id) references jbrowse (jbrowse_id) on delete cascade INITIALLY DEFERRED,
...
create table jbrowse_track (
    ...
    foregin key (type_id) references cvterm (cvterm_id) on delete cascade INITIALLY DEFERRED,
...
COMMENT ON COLUMN track IS 'Json configuration of every track as specified in trackList.json file in jbrowse. The entire trackList.json 
is not stored as it can be generated by combining each value for each organism.';

COMMENT ON COLUMN type_id IS 'Foreign key linked to the ontology, makes sure each track configuration has a defined 
